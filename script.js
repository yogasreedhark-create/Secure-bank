...
function loginUser() {
    ...
    if (!allUsers[username]) {
        allUsers[username] = { otp: "", balance: 1000, transactions: [] };
    }
    // Generate OTP
...
function transferMoney() {
    ...
    if (!allUsers[receiver]) {
        allUsers[receiver] = { balance: 1000, transactions: [] };
    }
    // Deduct sender
...
if (window.location.pathname.includes("transactions.html")) {
    document.addEventListener("DOMContentLoaded", () => {
        ...
        allUsers[username].transactions.forEach(t => {
            ...
            li.innerHTML = `<strong>${t.type}</strong><br> Amount: ₹${t.amount}<br> <small>${t.time}</small>`;
            list.appendChild(li);
        });
    });
}
...
function downloadPDF() {
    ...
    let doc = new jsPDF();
    doc.setFontSize(16);
    doc.text(`Transaction Statement - ${username}`, 20, 20);

    let startY = 40;
    allUsers[username].transactions.forEach((t, index) => {
        doc.setFontSize(12);
        doc.text(`${index + 1}. ${t.type} | ₹${t.amount} | ${t.time}`, 20, startY + index * 10);
    });

    doc.save(`${username}_statement.pdf`);
}
